...
class ImportService {
  ...
  static Future<ImportResult> _processImportPlanComptable(
    List<int> bytes,
  ) async {
    ...
    final colNumero = _findCol(headers, [
      'n° compte',
      'numero compte',
      'numero_compte',
      'n°compte',
    ...
    final colIntitule = _findCol(headers, [
      'intitulé',
      'intitule',
      'libellé',
      'libelle',
    ...
    if (colNumero == -1 || colIntitule == -1) {
      throw Exception(
        'Colonnes requises introuvables : "N° Compte" et "Intitulé"',
      );
    }
  ...
  static Future<void> importTiers({
    required BuildContext context,
    required VoidCallback onSuccess,
  }) async {
    final result = await _pickExcelFile(context);
    ...
    try {
      final importResult = await _processImportTiers(result);
      if (context.mounted) Navigator.pop(context);
      if (context.mounted) {
        _showResultDialog(context, importResult);
        if (importResult.inserted > 0) onSuccess();
  ...
  static Future<ImportResult> _processImportTiers(List<int> bytes) async {
    final excel = Excel.decodeBytes(bytes);
    final sheet = excel.sheets.values.first;
    final rows = sheet.rows;
    if (rows.isEmpty) {
    ...
    final colType = _findCol(headers, ['type']);
    final colNif = _findCol(headers, ['nif', 'numéro fiscal', 'numero fiscal']);
    final colAdresse = _findCol(headers, ['adresse']);
    final colCompteCollectif = _findCol(headers, [
      'compte comptable',
      'compte collectif',
      'compte associé',
      'compte associe',
      'numéro compte comptable',
      'numero compte comptable',
    ]);

    if (colNumero == -1 || colIntitule == -1 || colCompteCollectif == -1) {
      throw Exception(
        'Colonnes requises introuvables : "N° Compte", "Intitulé" et "Compte comptable"',
      );
    }

    final comptes = await AuthService.getComptes();
    final comptesByNumero = <String, Compte>{
      for (final compte in comptes) compte.numeroCompte.trim(): compte,
    };

    final tiersExistants = await AuthService.getTiers();
    final tiersByNumero = <String, Tiers>{
      for (final tiers in tiersExistants) tiers.numeroCompte.trim(): tiers,
    };
    ...
    for (int i = 1; i < rows.length; i++) {
      ...
      final numero = _cell(row, colNumero).trim();
      final intitule = _cell(row, colIntitule).trim();
      final compteCollectifNumero = _cell(row, colCompteCollectif).trim();
      final nif = colNif != -1 ? _cell(row, colNif).trim() : '';
      final adresse = colAdresse != -1 ? _cell(row, colAdresse).trim() : '';

      if (numero.isEmpty && intitule.isEmpty && compteCollectifNumero.isEmpty) {
        continue;
      }
      ...
      if (numero.isEmpty) {
        rowErrors.add('N° Compte manquant');
      }
      if (intitule.isEmpty) {
        rowErrors.add('Intitulé manquant');
      }
      if (compteCollectifNumero.isEmpty) {
        rowErrors.add('Compte comptable manquant');
      }

      final compteCollectif =
          compteCollectifNumero.isEmpty
              ? null
              : comptesByNumero[compteCollectifNumero];

      if (compteCollectifNumero.isNotEmpty && compteCollectif == null) {
        rowErrors.add(
          'compte comptable "$compteCollectifNumero" introuvable dans le plan comptable',
        );
      } else if (compteCollectif != null && !compteCollectif.liaisonTiers) {
        rowErrors.add(
          'compte comptable "$compteCollectifNumero" non autorisé pour un tiers (liaison tiers désactivée)',
        );
      }

      final parsedType =
          colType != -1 ? _parseTypeTiers(_cell(row, colType).trim()) : null;
      final inferredType = _inferTypeTiersFromNumero(numero);
      final type = parsedType ?? inferredType ?? TypeTiers.autre;

      if (rowErrors.isNotEmpty) {
        errors.addAll(
          rowErrors.map((error) => 'Ligne $lineNum ($numero) : $error'),
        );
        continue;
      }

      try {
        final existing = tiersByNumero[numero];
        if (existing != null) {
          await AuthService.updateTiers(
            ...
            compteCollectif: compteCollectifNumero,
            nif: nif.isEmpty ? null : nif,
        ...
        final refreshed = Tiers(
          id: existing?.id ?? numero,
          numeroCompte: numero,
          intitule: intitule,
          type: type,
          compteCollectif: compteCollectifNumero,
        ...
        tiersByNumero[numero] = refreshed;
        inserted++;
      } catch (e) {
        errors.add('Ligne $lineNum ($numero) : ${e.toString()}');
      }
  ...
  static Future<ImportResult> _processImportJournaux(List<int> bytes) async {
    ...
    final colType = _findCol(headers, ['type']);
    final colCompte = _findCol(headers, [
      'compte trésorerie',
      'compte tresorerie',
      'compte_tresorerie',
    ]);
    final colAnalyt = _findCol(headers, [
      'saisie analytique',
      'saisie_analytique',
      'analytique',
    ]);
    ...
    final existingCodes =
        existing.map((j) => j.code.trim().toUpperCase()).toSet();
    ...
    for (int i = 1; i < rows.length; i++) {
      ...
      if (type != 'financier' && type != 'non_financier') {
        errors.add(
          'Ligne $lineNum ($code) : Type invalide — doit être "Financier" ou "Non Financier"',
        );
        continue;
      }

      final compteTresorerie =
          colCompte != -1
              ? _cell(row, colCompte).trim().isEmpty
                  ? null
                  : _cell(row, colCompte).trim()
              : null;
  ...
  static TypeTiers? _parseTypeTiers(String raw) {
    final value = raw.trim().toLowerCase();
    if (value.isEmpty) return null;

    if (value.contains('client') ||
        value.contains('adhérent') ||
        value.contains('adherent')) {
      return TypeTiers.client;
    }
    if (value.contains('fournisseur')) {
      return TypeTiers.fournisseur;
    }
    if (value.contains('salar') || value == 'salarié' || value == 'salarie') {
      return TypeTiers.salarie;
    }
    if (value.contains('autre')) {
  ...
  static TypeTiers? _inferTypeTiersFromNumero(String numeroCompte) {
    if (numeroCompte.isEmpty || numeroCompte.length < 2) return null;

    switch (numeroCompte.substring(0, 2)) {
      case '41':
        return TypeTiers.client;
      case '40':
        return TypeTiers.fournisseur;
  ...
  static String? _parseNatureCompte(String raw, String numeroCompte) {
    const labelToDb = {
      ...
      'bilan (etat et collectivités publiques)':
          'bilan_etat_collectivites_publiques',
    ...
    const validDb = {
      'bilan_ressources_durables',
      'bilan_actif_immobilise',
      'bilan_stocks',
      'bilan_fournisseurs',
      'bilan_adherents_clients_usagers',
      'bilan_personnel',
      'bilan_organismes_sociaux',
      'bilan_etat_collectivites_publiques',
      'bilan_autres_tiers',
      'bilan_banque',
      'bilan_caisse',
      'bilan_autres_tresoreries',
